...
const Reports = () => {
  ...
  const [reportedUserLoading, setReportedUserLoading] =
    useState<boolean>(false);
  const [reportedUsers, setReportedUsers] = useState<IReportedUsers[]>([]);
  const [reportedReviewLoading, setReportedReviewLoading] =
    useState<boolean>(false);
  ...
  const handleDeleteReportForUser = async (reportId: number) => {
    const [err, data] = await to(
      reportsService.deleteReportForUser(reportId, token),
    );
  ...
  const handleDeleteReportForReview = async (reportId: number) => {
    const [err, data] = await to(
      reportsService.deleteReportForReview(reportId, token),
    );
    if (err) return showErrorMessage(err);
    showSuccess(data.message);
    ...
    return (
      ...
      <div className="flex gap-5 flex-row justify-between">
        ...
        <Button
          label="Raporu Sil"
          severity="help"
          onClick={() => handleDeleteReportForReview(reportId)}
        />
      </div>
    );
  };
    ...
    return (
      <div className="flex flex-row flex-wrap gap-3">
        ...
        <div className="flex flex-wrap">
          ...
          <a
            href={`http://localhost:5173/admin/users?userId=${user.id}`}
            target="_blank"
    ...
    <div className="flex flex-col gap-4 w-full">
      <div className="col-span-1">
        <Fieldset legend="Raporlanan Kullanıcılar" toggleable={true}>
          {msgsRepUser && (
            <Messages ref={msgsRepUser} className="w-1/2 ml-24" />
          )}
          {reportedUserLoading ? (
            <ProgressSpinner />
          ) : (
      ...
      <div className="col-span-1">
        <Fieldset legend="Raporlanan Yorumlar" toggleable={true}>
          {msgsRepReview && (
            <Messages ref={msgsRepReview} className="w-1/2 ml-24" />
          )}
