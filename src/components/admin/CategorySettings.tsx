...
const CategorySettings = () => {
  ...
  const [updateCategoryActive, setUpdateCategoryActive] =
  ...
  const handleUpdateCategory = async (id: number, val: any) => {
    ...
    const [err, data] = await to(
      categoryService.updateCategoryById(id, val, token),
    );
    if (err) return showErrorMessage(err);
    showSuccess(data.message);
    handleGetAllCategories();
    ...
    <>
      ...
      <div className="flex flex-col gap-4 w-full">
        ...
        <div className="w-full text-center">
          <Button
            className="p-button-rounded p-button-info sm:w-64 w-full"
            icon="pi pi-pencil"
            onClick={() => {
        ...
        {!updateCategoryActive && (
          <>
            ...
            <span className="text-sm text-gray-500">
              Eğer üst kategori seçilmezse kategori kök kategori olarak
              eklenecektir.
            </span>
          </>
        )}
            ...
            {updateCategory && (
              <div className="flex flex-col gap-3">
                ...
                <InputText
                  placeholder="Yeni Kategori Adı*"
                  className={
                    updateCategoryFormik.touched.categoryName &&
