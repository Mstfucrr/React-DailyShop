...
const UserOrders = () => {
  ...
  const legendTemplate = (order: IOrder) => {
    return <div className="flex md:flex-row flex-col sm:items-center !text-sm justify-between text-white">
      <span className="ml-1">Sipariş No: <span className="text-primary">{order.OrderNumber}</span></span>
      <span className="ml-1">Tarih: <span className="text-primary">{order.date}</span></span>
      <span className="ml-1">Toplam: <span className="text-primary">{order.totalPrice} ₺</span></span>
      <span className="ml-1">Durum: <span className="text-primary">{orderStatus.find((item) => item.value === order.status)?.label}</span></span>
    </div>
    ...
    return (
      <div className="flex m-2 shadow-lg px-5 py-8 w-full">
        <div className="flex flex-row flex-wrap gap-5 items-center">
          <Link className="sm:w-1/3 w-full hover:scale-110 transition duration-300" to={`/product/${orderItem.product.id}`}>
            <img src={orderItem.product.image} alt={orderItem.product.name} className="w-full h-auto object-scale-down" />
          </Link>
          <div className="">
            <div className="product-name">{orderItem.product.name}</div>
            <Rating value={orderItem.product.rating} readOnly cancel={false}
              pt={{
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-col gap-7">
        {orders?.map((order) => {
          return <Fieldset className="w-full flex flex-col shadow-lg gap-9" toggleable
            key={"order-" + order.id}
          ...
          >
            <div className="flex flex-col gap-6 sm:max-w-none max-w-xs w-full">
              ...
              <div className="flex flex-col w-full gap-7 overflow-x-auto">
                ...
                <Steps model={steps} readOnly activeIndex={
