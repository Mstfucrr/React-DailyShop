...
const Account = () => {
  ...
  useEffect(() => {
    ...
    AccountTabs.USER_INFO,
    AccountTabs.USER_ORDERS,
    AccountTabs.USER_PRODUCTS,
    tab,
  ]);
  ...
  const renderTabsButtons = (label: string, value: AccountTabs) => (
    <motion.a
      href={`/account/${value}`}
      className={`text-center py-[15px] border border-solid rounded-2xl text-lg font-medium
                    ${
                      activeTab == value
                        ? "bg-primary text-white border-[#ddd]"
                        : "bg-white text-primaryDark border-primary"
                    }                    
      ...
      <div className="lg:w-9/12 md:w-11/12 w-full mx-auto">
        <div className="w-full flex md:flex-row flex-col gap-x-7">
          <div className="basis-5/12 w-full px-[15px] my-10">
            <h3 className="text-4xl my-4 text-primaryDark ">Hesabım</h3>
            <motion.div
          ...
          <div className=" w-full">
            <AnimatePresence>
              ...
              {!loading && user && activeTab == AccountTabs.USER_INFO && (
                <UserInformation user={user} />
              )}
              {!loading && user && activeTab == AccountTabs.USER_ORDERS && (
                <UserOrders />
              )}
              {!loading && user && activeTab == AccountTabs.USER_PRODUCTS && (
                <UserProducts />
              )}
            </AnimatePresence>
          </div>
